...
fn main() {
    ...
    let mut n = neural_network::neurons::Network::new(vec![2, 8, 1], 0.0000001, vec![Activation::Linear, Activation::Linear, Activation::Linear], Loss::MSE);
    let mut rng = rand::thread_rng();
    for i in 0..1000000 {
        let x: Vec<Vec<f32>> = (0..1).map(|_| vec![rng.gen_range(0..1000) as f32, rng.gen_range(0..1000) as f32]).collect();
        let y: Vec<Vec<f32>> = (0..1).map(|i| vec![x[i][0] + x[i][1]]).collect();
        n.fit(x, y);
    }
    println!("prediction: {}, result: {}", n.predict(vec![503.9, 407.33]).unwrap()[0], 503.9+407.33);
    // println!("{:?}", n);
    // chess_pvp();
}
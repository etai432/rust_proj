...
pub fn run() {
    ...
    let mut root = String::from(abs_path.to_str().unwrap().replace(r"/", r"\").split(r"\").collect::<Vec<&str>>()[0]).replace(r"/", r"\");
    root.push_str(r"\");
    ...
    while input.ne("done") {
        ...
        if in_vec.len() > 0 {
            match in_vec[0] {
                "cmd" => println!("commands: ls, cd, root, env, this, current, back, cdp, cdr, rename, done"),
                ...
                "env" => current = (String::from(abs_path.to_str().unwrap()) + r"\").replace(r"/", r"\"),
                "this" => current = (String::from(abs_path.to_str().unwrap()) + r"\" + file!() + r"\").replace(r"/", r"\"),
